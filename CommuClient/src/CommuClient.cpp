...
int main() {
	...
	sprintf(szPath, "%s/%s", curdir, SETTINGFILE);
	char buf[BUFSIZ] = { 0 };
    printf("connected to server\n");

    char szIp[TINYSIZE] = { 0 };
    RegTool::GetPrivateProfileString(INFO, SVRIP, szIp, szPath,  "127.0.0.1");
    int port;
    RegTool::GetPrivateProfileInt(INFO, PORT, port, szPath, ADB_TCP_SERVER_PORT);

	start_client_socket(szIp, port, &global_client_sockfd);
